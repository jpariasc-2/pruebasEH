...
$HAS_ANTSPOOF		= query("/runtime/func/anti_spoof");
...
if ($HAS_ANTSPOOF=="1")
{
	/* Anti-spoof (ARP)*/
	$db_dirty_arp=0;
	if ($arp_enable!="1") {$arp_enable=0;}
	if (query("/security/arp/enable")!=$arp_enable) {set("/security/arp/enable", $arp_enable); $db_dirty_arp++;}
	...
	if (query("/runtime/func/nat_endpoint")==1)
	{
		if (query("/stun/type/udp")!=$udp_endpoint_type) {set("/stun/type/udp", $udp_endpoint_type); $db_dirty_wan++;}
		if (query("/stun/type/tcp")!=$tcp_endpoint_type) {set("/stun/type/tcp", $tcp_endpoint_type); $db_dirty_wan++;}
	}
	...
	if($db_dirty_arp > 0)	{$SUBMIT_STR=$SUBMIT_STR.";submit RG_FIREWALL";}
	if($db_dirty_wan > 0)	{$SUBMIT_STR=$SUBMIT_STR.";submit WAN";}
...
if(query("/runtime/func/nat_endpoint")==1)
{
	$cfg_udp_endpoint	=	query("/stun/type/udp");
	$cfg_tcp_endpoint	=	query("/stun/type/tcp");
}
/* --------------------------------------------------------------------------- */
?>
...
if ($HAS_ANTSPOOF=="1")
{
	echo "	f.arp_enable.checked = ";
	if(query("/security/arp/enable")=="1"){echo "true;\n";}else{echo "false;\n";}
}
?>
	/* NAT Endpoint Filtering	*/
	<?
		if(query("/runtime/func/nat_endpoint")==1)
		{
			if($cfg_udp_endpoint =="1"){echo "f.udp_endpoint_type_1.checked=true;\n";}
			else if($cfg_udp_endpoint == "2"){echo "f.udp_endpoint_type_2.checked=true;\n";}
			else if($cfg_udp_endpoint == "3"){echo "f.udp_endpoint_type_3.checked=true;\n";}
			else {echo "f.udp_endpoint_type_1.checked=true;\n";}
			
			if($cfg_tcp_endpoint =="1"){echo "f.tcp_endpoint_type_1.checked=true;\n";}
			else if($cfg_tcp_endpoint == "2"){echo "f.tcp_endpoint_type_2.checked=true;\n";}
			else if($cfg_tcp_endpoint == "3"){echo "f.tcp_endpoint_type_3.checked=true;\n";}
			else {echo "f.tcp_endpoint_type_1.checked=true;\n";}			
		}	
	?>
...
function check()
{
	...
	for(i=1; i < rules; i++)
	{
		...
		if( get_obj("fw_enable_"+i).checked && is_blank(tmp_fw_description) )
		{
...
?>
		<div class="box">
			...
			<?if($HAS_ANTSPOOF!="1"){echo "<!--\n";}?>
			<tr>
				<td <?=$td_width0?>><?=$m_enable_arp?>&nbsp;:</td>
				<td <?=$td_width2?>>&nbsp;<input type="checkbox" name="arp_enable" id=arp_enable value="1"></td>
			</tr>
			<?if($HAS_ANTSPOOF!="1"){echo "-->\n";}?>
			</table>
		</div>
<!--__________________________________ Add	NAT Endpoint Filtering _________________________________-->
<?if(query("/runtime/func/nat_endpoint")!=1){echo"<!--";}?>
		<div class="box">
			<h2><?=$m_title_nat_endpoint?></h2>
			<table border=0>
			<tr>
				<td <?=$td_width0?>>&nbsp;<br><?=$m_udp_endpoint?>&nbsp;:<br>&nbsp;<br></td>
				<td>
					&nbsp;<input type="radio" id="udp_endpoint_type_3" name="udp_endpoint_type" value="3">&nbsp;<?=$m_endpoint_independent?><br>
					&nbsp;<input type="radio" id="udp_endpoint_type_2" name="udp_endpoint_type" value="2">&nbsp;<?=$m_address_restricted?><br>
					&nbsp;<input type="radio" id="udp_endpoint_type_1" name="udp_endpoint_type" value="1">&nbsp;<?=$m_port_address?><br>
				</td>
			</tr>
			<tr><td>&nbsp;<br></td><td>&nbsp;<br></td></tr>
			<tr>
				<td <?=$td_width0?>>&nbsp;<br><?=$m_tcp_endpoint?>&nbsp;:<br>&nbsp;<br></td>
				<td>
					&nbsp;<input type="radio" id="tcp_endpoint_type_3" name="tcp_endpoint_type" value="3">&nbsp;<?=$m_endpoint_independent?><br>
					&nbsp;<input type="radio" id="tcp_endpoint_type_2" name="tcp_endpoint_type" value="2">&nbsp;<?=$m_address_restricted?><br>
					&nbsp;<input type="radio" id="tcp_endpoint_type_1" name="tcp_endpoint_type" value="1">&nbsp;<?=$m_port_address?><br>
				</td>					
			</tr>
			</table>
		</div>
<?if(query("/runtime/func/nat_endpoint")!=1){echo"-->";}?>
<!--_________________________________ Add end ________________________________________________________-->
		<div class="box">
