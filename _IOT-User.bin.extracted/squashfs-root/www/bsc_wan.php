...
if ($ACTION_POST!="")
{
  ...
  echo "<!--\n";
	...
	if ($l2tp_phy!="1" && query("/wan/rg/inf:1/l2tp/physical")==1)
	{
		$dirty++;
		set("/wan/rg/inf:1/l2tp/physical","0");
/*
		set("/wan/rg/inf:1/l2tp/mppe/enable", "0");
		$l2tp_mppe=0;
*/
	}
	...
	else if ($ACTION_POST == "DHCP")
	{
		...
		if (query($entry."unicast")		!= $unicast)		{$dirty++; set($entry."unicast", $unicast);}
		if (query($entry."clonemac")!= $clonemac)	{$dirty++; set($entry."clonemac", $clonemac);}
		if (query($entry."mtu")		!= $mtu)		{$dirty++; set($entry."mtu", $mtu);}
		$entry="/dnsrelay/server/";
		if (query($entry."primarydns")	!=$dns1){$dirty++; set($entry."primarydns", $dns1);}
	...
	else if ($ACTION_POST == "PPTP" || $ACTION_POST == "L2TP")
	{
		...
		else
		{
			...
			if (query($entry."physical")!=$l2tp_phy)        {$dirty++; set($entry."physical", $l2tp_phy);}
		}
		if (query($entry."mode")!=$mode)			{$dirty++; set($entry."mode", $mode);}
		if ($mode == "1")
		{
	...
	else if ($dirty > 0)    {set("/sys/configdirty", "1");$SUBMIT_STR=";submit WAN";}
...
$cfg_dhcp_unicast		= query("unicast");
...
function on_change_wan_type()
{
	...
	case "5":		get_obj("show_l2tp").style.display		= get_obj("org_l2tp").style.display = ""; break;
	...
	case "r_pppoe":
		...
		?>
		break;
	case "r_l2tp":	get_obj("show_l2tp").style.display		= "";
					get_obj("russia_l2tp").style.display	= "";
		break;
...
function init_dhcp()
{
	...
	<?
		...
		{
			if ($cfg_dhcp_unicast=="1") 	{echo "f.unicast.checked = true;\n";} 
			else							{echo "f.unicast.checked = false;\n";}
		}
	?>
	//////////////////
	init_dh8021x();
...
function check_pptp_l2tp(form)
{
	...
	if(get_obj("wan_type").value=="r_l2tp")
	{
		get_obj("l2tp_phy").disabled = false;
	}
	else
	{
		get_obj("l2tp_phy").disabled = true;
	}
			...
			<tr>
				...
				<td class="l_tb">&nbsp;
					<select name="wan_type" id="wan_type" onChange="on_change_wan_type()">
						...
						<option value="r_l2tp"<? if($r_mode=="r_l2tp")  {echo " selected";}?>><?=$m_russia_l2tp?></option>
...
<!--*********************************************-->
	...
	<!-- DHCP  -->
		...
		<?
			...
			{
			echo	"<tr>\n";
			echo	"<td class=\"r_tb\" width=150>".$m_use_unicast." :</td>\n";
			echo	"<td class=\"l_tb\">&nbsp;\n";
			echo	"<input type=\"checkbox\" id=\"unicast\" name=\"unicast\" value=\"1\">".$m_unicast_dsc."\n";
			echo	"</td>\n";
			echo	"</tr>\n";
			}
		?>
...
<?						$PREFIX		= "\t\t\t\t\t";
	...
	<!-- L2TP  -->
		<div class="box" id="show_l2tp" style="display:none">
			<div id="org_l2tp" style="display:none"><h2><?=$m_title_l2tp?></h2></div>
			<div id="russia_l2tp" style="display:none"><h2><?=$m_title_russia_l2tp?></h2></div>
		...
		<input type=hidden name="l2tp_phy" id="l2tp_phy" value="1" disabled="true">
		<p><?=$m_desc_l2tp?></p>
		<table cellSpacing=1 cellPadding=1 width=525 border=0>
		<tr>
			<td class="r_tb" width=150></td>
