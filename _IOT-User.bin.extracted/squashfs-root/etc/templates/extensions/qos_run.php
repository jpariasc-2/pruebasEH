...
if ($generate_start==1)
{
	...
	if ( $QOS_ENABLE!=1 && $QOS_ENABLE!=2 ){ echo "killall pp.sh; echo QOS is disabled. \n";exit; }
    ...
    if($PRIO0_MAX == 0) {$PRIO0_MAX = 1;}
    if($PRIO1_MAX == 0) {$PRIO1_MAX = 1;}
    if($PRIO2_MAX == 0) {$PRIO2_MAX = 1;}
    if($PRIO3_MAX == 0) {$PRIO3_MAX = 1;}
    if($PRIO0_MIN == 0) {$PRIO0_MIN = 1;}
    if($PRIO1_MIN == 0) {$PRIO1_MIN = 1;}
    if($PRIO2_MIN == 0) {$PRIO2_MIN = 1;}
    if($PRIO3_MIN == 0) {$PRIO3_MIN = 1;}
	...
	/*--------------------------------------------------------*/
	$UBICOM = query("/qos/ubicom");
	///if( $UBICOM == "1" && $UPSTREAM == "102400" ){ echo "killall ubcom;ubcom &"; }
	if($UBICOM != "1") { echo "killall pp.sh; \n"; }
	else
	{
		if( $UPSTREAM == "102400" )  { echo "killall pp.sh;/etc/templates/extensions/pp.sh & \n"; }
	}
	/*--------------------------------------------------------*/
