...
if ($wanmode == 3)
{
	 if($wanif != "" || $wanif2 != "")
	 {
		 ...
		 echo "iptables -t mangle -A FORWARD -p tcp --tcp-flags SYN SYN -j TCPMSS --clamp-mss-to-pmtu\n";
	 }
 }
if ($wanmode == 4 )
{
...
if ($wanmode == 6)
{
     if($wanif != "" || $wanif2 != "")
	 {
		...
		echo "iptables -t mangle -A FORWARD -p tcp --tcp-flags SYN SYN -m -j TCPMSS --clamp-mss-to-pmtu\n";
	 }
}	
...
if(query("flowmeter") > 0)
{ echo "iptables -A FORWARD -j FOR_FLOWMETER\n"; }
/* POSTROUTING */
if (query("vrtsrv") > 0 || query("dmz") > 0)
{ 
...
/* To avoid LAN IP Packet flow into WAN */
if ($wanif!="") { echo "iptables -t mangle -A POSTROUTING -o ".$wanif." -m state --state INVALID -j DROP\n"; }
if ($wan2if!=""){ echo "iptables -t mangle -A POSTROUTING -o ".$wan2if." -m state --state INVALID -j DROP\n"; }
?>
# flush_main.php <<<
