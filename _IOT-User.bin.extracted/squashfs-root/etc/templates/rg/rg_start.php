...
# Add custom chains
iptables -t mangle	-N PRE_TCMARK
...
# DOS
iptables -t nat		-A PRE_DOS -p tcp --syn -m limit --limit 10/s --limit-burst 50 -j RETURN
iptables -t nat		-A PRE_DOS -p tcp --syn <?=$LOG_STRING?> 'ATT:002[TCP-FLOODING]:'
iptables -t nat		-A PRE_DOS -p tcp --syn -j DROP
...
iptables -t nat		-A PRE_DOS -p icmp --icmp-type echo-request <?=$LOG_STRING?> 'ATT:002[PING-FLOODING]:'
iptables -t nat		-A PRE_DOS -p icmp --icmp-type echo-reply <?=$LOG_STRING?> 'ATT:002[PING-FLOODING]:'
iptables -t nat		-A PRE_DOS -p icmp --icmp-type echo-request -j DROP
iptables -t nat		-A PRE_DOS -p icmp --icmp-type echo-reply -j DROP
# SPI
